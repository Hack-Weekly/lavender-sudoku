...
export const Signup = () => {
  ...
  const handleSignUp = async() => {
    try{
      await register({username : username, email : email, password : password, password2 : password})
      navigate("/");
    }catch(error){
      setIsInvalid(true);
  ...
  return (
    <div className="w-screen h-screen flex items-center justify-center bg-gradient-to-b from-blue-300 to-blue-500">
      <div className="h-4/5 w-2/3 bg-white rounded-lg flex flex-col items-center p-8 shadow-lg gap-4">
        ...
        <div className="w-full max-w-md flex flex-col gap-4" onChange={()=>{setErrorMessage("");setIsInvalid(false)}}>
          ...
          <button onClick={handleSignUp} className="bg-blue-600 text-white px-8 py-3 rounded-md text-lg font-semibold hover:bg-blue-700 transition duration-300 ease-in-out">
            Sign up
          </button>
